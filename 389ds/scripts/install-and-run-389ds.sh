...
# TODO -- Can these not be exported as DOCKER ENV variables ??
...
load_example_com() {
  ...
  ldapadd -x -D"$ROOT_DN" -w${DIRSRV_MANAGER_PASSWORD} -f /tmp/users_and_groups.ldif
...
if [ ! -d ${BASEDIR} ]; then
  ...
  echo "Directory server already setupm running server"
fi
